...
import { Sidebar, Volume2, Play, RotateCcw, Check, X, ChevronLeft, ChevronRight, Lamp } from 'lucide-react';
                        ...
                        >
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-sm">
                                    <span>Câu {currentSentenceIndex + 1} / {sentences.length}</span>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="text-lg leading-relaxed">
                                            {sentences[currentSentenceIndex]?.text.split(' ').map((word, index) => {
                                                ...
                                                const isMissing = sentences[currentSentenceIndex]?.missingWords.some(missingWord => 
                                                    missingWord.replace(/[.,!?;:]/g, '') === cleanWord
                                                );
                                                const missingWordIndex = sentences[currentSentenceIndex]?.missingWords.findIndex(missingWord => 
                                                    missingWord.replace(/[.,!?;:]/g, '') === cleanWord
                                                ) || 0;
                                                    ...
                                                    <span key={index}>
                                                        {isMissing ? (
                                                            <span className="inline-block mx-1">
                                                                <input
                                                                    type="text"
                                                                    value={userAnswers[currentSentenceIndex]?.[missingWordIndex] || ''}
                                                                    onChange={(e) => handleUserInput(e.target.value, missingWordIndex)}
                    ...
                    {currentStep === 'practice' && showResults && (
                        ...
                        >
                            <div className="text-center space-y-2">
                                ...
                                <div className={`text-2xl font-bold ${sentences[currentSentenceIndex]?.isCorrect ? 'text-green-500' : 'text-red-500'}`}>
                            ...
                            <div className="space-y-4">
                                <div
                                    className={`p-4 rounded-lg border ${sentences[currentSentenceIndex]?.isCorrect ? 'border-green-500 bg-green-900/20' : 'border-red-500 bg-red-900/20'
                                        }`}
                                >
